...
it(`encrypts and decrypts using the second password`, async () => {
  ...
  const cipherText = await taskToPromise(
    core.encryptWithSecondPassword(options, message)
  )

  expect(
    await taskToPromise(core.decryptWithSecondPassword(options, cipherText))
  ).toEqual(message)
})
