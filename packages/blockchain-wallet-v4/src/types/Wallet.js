...
export const isValidSecondPwd = curry(async (imports, password, wallet) => {
  if (isDoubleEncrypted(wallet)) {
    ...
    let computedHash = await imports.securityProcess.computeSecondPasswordHash({
      iterations,
      password
    })
...
const applyCipher = curry(
  returnTask(async (imports, wallet, password, f, value) => {
    let it = selectIterations(wallet)
    switch (true) {
      case !isDoubleEncrypted(wallet):
        return value
      case await isValidSecondPwd(imports, password, wallet):
        return f(it, imports, password, value)
      default:
...
export const importLegacyAddress = curry(
  (wallet, key, createdTime, password, bipPass, { imports, network, api }) => {
    ...
    return Address.fromString(key, createdTime, null, bipPass, { network, api })
      .chain(checkIfExists)
      .chain(applyCipher(imports, wallet, password, Address.encrypt))
      .map(appendAddress)
  }
)
...
export const newHDWallet = curry((imports, mnemonic, password, wallet) => {
  let hdWallet = HDWallet.createNew(mnemonic)
  let appendHdWallet = curry((w, hd) =>
    over(hdWallets, list => list.push(hd), w)
  )
  return applyCipher(imports, wallet, password, HDWallet.encrypt, hdWallet).map(
...
export const newHDAccount = curry(
  ({ imports, label, secondPassword, network }, wallet) => {
    ...
    return validateSecondPwd(imports, secondPassword, wallet)
      .chain(() =>
        promiseToTask(
          HDWallet.generateAccount({
            deriveBIP32Key: curry(imports.securityProcess.deriveBIP32Key)(
              credentials
            ),
            index,
            label,
      ...
      .chain(applyCipher(imports, wallet, secondPassword, HDAccount.encrypt))
...
export const getPrivateKeyForAddress = curry(
  (imports, wallet, password, addr) => {
    let address = AddressMap.selectAddress(addr, selectAddresses(wallet))
    return applyCipher(imports, wallet, password, Address.decrypt, address).map(
      a => a.priv
    )
  }
)
...
export const encrypt = returnTask(
  curry(async ({ securityProcess }, password, wallet) => {
    if (isDoubleEncrypted(wallet)) {
      return wallet
    } else {
      let iterations = selectIterations(wallet)
      ...
      let enc = returnTask(
        curry(securityProcess.encryptWithSecondPassword)({
          iterations,
          password
        })
      )

      let hash = await securityProcess.computeSecondPasswordHash({
        iterations,
        password
      })
...
export const decrypt = curry((imports, password, wallet) => {
  if (isDoubleEncrypted(wallet)) {
    let iterations = selectIterations(wallet)

    let dec = returnTask(
      curry(imports.securityProcess.decryptWithSecondPassword)({
        iterations,
        password
      })
    )
    ...
    return validateSecondPwd(imports, password, wallet)
...
const validateSecondPwd = returnTask(async (imports, password, wallet) => {
  if (await isValidSecondPwd(imports, password, wallet)) {
    return wallet
  } else {
    throw new Error('INVALID_SECOND_PASSWORD')
  }
...
export const getHDPrivateKeyWIF = curry(
  (imports, keypath, secondPassword, network, wallet) => {
...
export const getLegacyPrivateKey = curry(
  (imports, address, secondPassword, network, wallet) => {
    let priv = compose(
      Address.selectPriv,
...
export const getSeedHex = curry((imports, secondPassword, wallet) => {
  ...
  if (isDoubleEncrypted(wallet)) {
    return validateSecondPwd(imports, secondPassword, wallet).chain(() =>
      crypto.decryptSecPass(
        selectSharedKey(wallet),
        selectIterations(wallet),
        secondPassword,
...
export const getMnemonic = curry((imports, secondPassword, wallet) => {
  const eitherToTask = e => e.fold(Task.rejected, Task.of)
  const entropyToMnemonic = compose(
    eitherToTask,
    Either.try(BIP39.entropyToMnemonic)
  )
  const seedHex = getSeedHex(imports, secondPassword, wallet)
