...
export default ({ api, imports: { securityProcess } = {}, networks }) => {
  ...
  const createRoot = function * ({ password }) {
    try {
      ...
      const metadata = yield call(securityProcess.deriveBIP32Key, credentials, {
  ...
  const fetchRoot = function * (secondPasswordSagaEnhancer) {
    try {
      ...
      const entropy = yield call(securityProcess.credentialsEntropy, { guid })
